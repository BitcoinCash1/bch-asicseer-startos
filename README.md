...
- **Shares (Σdiff)** — per-worker accepted share WORK, as asicseer-pool stores it internally (`worker->shares += diff` on every accepted share). This is a diff-1–weighted SUM, not a raw submission count, and it's the only accepted-share metric the pool exposes per-worker. That's why it can read e.g. `182.00 M` while your ASIC's own UI shows `Accepted: 6`. The two numbers are measuring different things and neither is wrong — they just aren't directly comparable.
  - Why no integer "Accepted" / "Rejected" count columns? Because asicseer-pool's source (ckpool-lineage `stratifier.c`) simply does not maintain a per-worker submission counter or a per-worker rejected counter — only the diff-weighted accumulator. Exposing one of the diff-weighted fields as a pseudo-count would be dishonest, so we removed those columns. For real submission counts, query your miner's own cgminer-compatible API on port `4028` (`{"command":"summary"}` returns `Accepted`, `Rejected`, `Hardware Errors`, etc.).
