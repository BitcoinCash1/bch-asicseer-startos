...
- *No "Rejected" column.* asicseer-pool only accounts rejects pool-wide (`stats.accounted_rejects`) — the stratifier keeps no per-client reject counter, so a per-worker Rejected column would always read 0 and be misleading. For real reject telemetry (stale / HW error / low-diff) query your ASIC's own cgminer API on port 4028.
